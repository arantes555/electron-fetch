...
const debug = function debug() {
  ...
  if (process.env.DEBUG_FETCH) (_console = console).log.apply(_console, ['[FETCH]'].concat(args));
};

global.debug = debug;

isReady.then(function () {
  return debug('app is ready');
});
...
function fetch(url$$1) {
  ...
  debug('requested fetch to url', url$$1);
  ...
  return isReady.then(function () {
    return new Promise(function (resolve$$1, reject) {
      ...
      req.on('close', function () {
        ...
        debug.apply(undefined, ['Got close with'].concat(args));
      });
      ...
      req.on('abort', function () {
        ...
        debug.apply(undefined, ['Got abort with'].concat(args));
      });
      ...
      req.on('finish', function () {
        ...
        debug.apply(undefined, ['Got finish with'].concat(args));
      });
      ...
      req.on('login', function () {
        ...
        debug.apply(undefined, ['Got login with'].concat(args));
      });
